...
class TestDynamicalSystem():
    ...
    def _check_1D_traj(self, traj):
        ...
        for chain_id in range(traj.num_chains):
            assert traj.get_chain(chain_id).shape == (self.simulation_length, self.parameter_dim, 1), f"Chain shape {traj.get_chain(chain_id).shape} but expected {(self.simulation_length, 1)}"
    ...
    def _check_2D_traj(self, traj):
        ...
        for chain_id in range(traj.num_chains):
            assert traj.get_chain(chain_id).shape == (self.simulation_length, self.parameter_dim, 2), f"Chain shape {traj.get_chain(chain_id).shape} but expected {(self.simulation_length, 2)}"
