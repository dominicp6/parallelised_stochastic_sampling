...
3) Anisotropic overdamped Langevin dynamics, $\overrightarrow{dX}_t =(- \mathbf{M}\left(\overrightarrow{X}_t\right)\overrightarrow{\nabla}V(X_t) + \beta^{-1} \overrightarrow{\nabla} \cdot \mathbf{M}\left(\overrightarrow{X}_t\right))dt + \sqrt{2 \beta^{-1}}\mathbf{M}^{\frac{1}{2}}\left(\overrightarrow{X}_t\right) \overrightarrow{dW}_t$ 
